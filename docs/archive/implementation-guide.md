# SOP EDITH — Implementation Guide
...
Industrial supervisors already decide whether dangerous work can proceed. The information often exists — in SCADA, permits, maintenance systems — but nobody synthesizes it at decision time. **SOP EDITH** runs a structured **Operational Review**: rules surface the facts, AI explains what they mean together, the supervisor makes the call, and the system leaves a permanent audit record.
...
**Positioning (frozen):** This is not "another app to open." The decision already happens via paper, phone, WhatsApp, and SCADA. SOP EDITH structures and audits it.
