...
Recreate the Visakhapatnam coke-oven pattern from the problem context: gas rising + active hot-work permit + incomplete isolation + a worker in-zone. Each individual sensor stays *below its own critical alarm*, so single-sensor monitoring stays silent (false negative). SOP EDITH fuses the sub-critical signals, raises a **blocking** compound verdict **minutes/hours before** any single sensor would trip, cites **OISD / Factory Act / DGMS**, and shows the risk spreading on the plant map.
